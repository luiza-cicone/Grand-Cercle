...
\subsubsection{Exigences aux yeux de l'utilisateur}
La première chose qui fait qu'une personne utilise une application smartphone est la rapidité avec laquelle elle accède à l'information qu'elle veut. L'application doit donc être à la fois légère du point de vue place mémoire mais également minimiser le nombre de clics et de menus pour pour trouver les informations. Cela implique donc une certaine simplicité d'utilisation avec peu de fenêtres différentes, une information centralisée afin de rendre la navigation intuitive. \\
\indent L'ergonomie de l'application est également un réel moteur d'utilisation, le visuel étant la première impression laissée auprès de l'utilisateur. Le choix des préférences mentionné dans la partie précédente doit donc permettre à l'utilisateur d'améliorer l'aspect visuel de l'application, en choisissant les couleurs et les thèmes qui lui conviennent. \\
\indent L'utilisateur sera également sensible à la fréquence de mise à jour sur l'application. Les informations présentées seront donc changées régulièrement, afin de créer un certain dynamisme de la vie étudiante de Grenoble INP auprès des étudiants.

\subsubsection{Exigences annexes}
L'application doit répondre à un réel besoin des étudiants en matière d'information. Le présent cahier des charges étant basé sur les attentes des utilisateurs recueillies lors d'interviews préalables, il se peut qu'une partie des attentes du public visé soit passée inaperçu dans les nombreux sondages effectués. Un comité de pilotage interne via les membres de l'association du Grand Cercle sera effectué afin de limiter ces risques.Néanmoins, il faudra assurer une maintenance de l'application, guidée par les réactions des utilisateurs. Cette maintenance sera rendue possible par des documents de conception dûment complétés, ainsi que par un code commenté. 
